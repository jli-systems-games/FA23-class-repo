        ...
        for (int i = 0; i < spawnAmount; i++)
        {
            ...
            while (viable == false)
            {
                ...
                foreach (Vector3 previous in previousLocations)
                {
                    ...
                    if(Mathf.Abs(xOffset) <= npcDistance &&
                        Mathf.Abs(yOffset) <= npcDistance)
                    {
                        viable = false;
                        badCounter++;
                        break;
            ...
            if (viable == true)
            {
                GameObject newNPC = Instantiate(
                    npcPrefab,
                    newVector, //new Vector3(x,y,0),
                    Quaternion.identity);

                previousLocations.Add(newVector);

                newNPC.GetComponent<NPCController>().npc = randomBase;
                newNPC.GetComponent<NPCController>().npc.npcCharisma
                    = Random.Range(8, 16);
                newNPC.GetComponent<NPCController>().npc.npcWeapon 
                    = weaponPool[Random.Range(0, weaponPool.Length - 1)];

                Debug.Log(newNPC.GetComponent<NPCController>().
                    npc.npcWeapon.weaponName);
        ...
        */
        ...
        for (int i = 0; i < listLength - 1; i++)
        {
            ...
            if (viable == true)
            {
                ...
                { 
                    newNPC.GetComponent<NPCController>().npc
                        = npcPool[tempIndex];
