...
{
    public static DataManager Instance { get; private set; } //this is a singleton (this is like a static variable but we don't have to make everything in this script static
    ...
    private void Awake()
    {
        if(Instance != null)
        {
            Debug.LogError("There can't be more than one data manager, idiot.");
        }
        Instance = this;
    }
    ...
    public void LoadGame()
    {
        ...
        foreach (IDataPersistence dataPs in _dataPersistences) //dataPs is just a random variable name, like "i" (dataPersistences)
        {
