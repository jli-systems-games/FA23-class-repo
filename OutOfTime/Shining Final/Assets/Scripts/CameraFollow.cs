...
public class CameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        if (target != null)
        {
            ...
            if(isVertical == true)
            {
                Vector3 adjustedPosition = new Vector3(0, (cameraY + 2), -10f);
            ...
            if (isVertical == false)
            {
                Vector3 adjustedPosition = new Vector3((cameraX + 2), 0, -10f);
                // Calculate the new position to move towards the target
                Vector3 targetPosition = adjustedPosition;
                Vector3 newPosition = Vector3.Lerp(transform.position, targetPosition, followSpeed * Time.deltaTime);
...
/*            if (isVertical == 2)
            {
                Vector3 adjustedPosition = new Vector3(cameraX, cameraY, -10f);
                // Calculate the new position to move towards the target
                Vector3 targetPosition = adjustedPosition;
                Vector3 newPosition = Vector3.Lerp(transform.position, targetPosition, followSpeed * Time.deltaTime);
